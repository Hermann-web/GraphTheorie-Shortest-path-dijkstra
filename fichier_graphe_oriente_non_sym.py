...
#avoir une matrice à coeff diagonaux nulls
for i in range(N):
	A[i][i]=0
	...
	while len(Q)!=0:
		...
		for somet in Q:
			if λ[somet] < distance_min:
				sommet_courant,distance_min = somet,λ[somet]

		#enlever le sommet couant de la liste des sommets à visitet
		print("\n")
		print("tentative d'enlever {} de Q".format(1+sommet_courant))
		Q.remove(sommet_courant); print("opération réussie: distance min entre {} et {} trouvé = {}".format(s+1,sommet_courant+1,distance_min))
		...
		for sommet_sucesseur in range(len(liste_poids_successeurs)):
			if liste_poids_successeurs[sommet_sucesseur]!=0:
				...
				if sommet_sucesseur not in D: 
					if sommet_sucesseur not in Q: 
						Q.append(sommet_sucesseur)
						print("le sommet {} a scanné".format(sommet_sucesseur+1))
					else:
						print("le sommet {} a été ajouté".format(sommet_sucesseur+1))
